...
if CURRENT_PYTHON < PYTHON_MINIMUM_VERSION:
    ...
    warnings.warn(
        f"Warning: Python {PYTHON_MINIMUM_VERSION[0]}.{PYTHON_MINIMUM_VERSION[1]} or later recommended. "
        f"You are using Python {sys.version}. "
        "Some features may not work correctly.",
        UserWarning,
    )

# Note: We cannot import from wifite during setup as it's not installed yet
...
long_description = (
    read_file("README.md")
    or """Wireless Network Auditor for Linux - Python 3.13.5 Edition.
...
setup(
    ...
    data_files=(
        ...
        if os.path.exists("wordlist-top4800-probable.txt")
        else []
    ),
    entry_points={
        "console_scripts": [
