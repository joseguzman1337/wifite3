...
try:
    ...
    def demo_network_detection():
        ...
        for privacy, cipher, auth, essid, description in test_networks:
            ...
            Color.pl('  {C}%-12s{W} | {G}%-15s{W} | {D}%s{W}' % (net_type, essid, description))
    ...
    def demo_vendor_detection():
        ...
        for mac, description in test_devices:
            ...
            Color.pl('  {C}%-8s{W} | {G}%-17s{W} | {D}%s{W}' % (vendor, mac, description))
    ...
    def demo_device_classification():
        ...
        test_clients = [
            ('00:1B:63:AA:BB:CC', 'iPhone_Network,HomeWiFi', 'iPhone connecting to home'),
            ('00:50:F2:11:22:33', 'laptop,windows_network', 'Windows laptop'),
            ('AA:BB:CC:DD:EE:FF', 'RingCam,IoT_Device,SmartHome', 'Ring doorbell camera'),
            ('34:23:87:DD:EE:FF', 'HP_Printer,PrintNet', 'Network printer'),
            ('00:0F:DE:AA:BB:CC', 'xbox_live,gaming_network', 'Xbox console'),
            ('00:16:EA:11:22:33', 'android_phone,mobile_hotspot', 'Android device'),
            ('00:1A:11:33:44:55', 'google_home,nest_cam', 'Google smart device')
        ]
        
        for mac, probed, description in test_clients:
            device_type = OpenMPI._detect_device_type(mac, probed)
            vendor = OpenMPI._detect_vendor_from_mac(mac)
            Color.pl('  {C}%-8s{W} | {G}%-8s{W} | {Y}%-17s{W} | {D}%s{W}' % (device_type, vendor, mac, description))
    ...
    def demo_intelligence_report():
        ...
        network_stats = {
            ...
            'hidden_networks': 4,
            'total_clients': 47,
            'unique_vendors': {'Apple', 'Samsung', 'Intel', 'Cisco', 'Google', 'Microsoft'},
            'channel_usage': {1: 8, 6: 12, 11: 5, 36: 3, 149: 2},
            'network_types': {'WPA2': 17, 'Open': 3, 'WPA': 2, 'Enterprise': 1, 'WPA3': 1, 'IoT': 1}
        }
        ...
        for channel, count in sorted(network_stats['channel_usage'].items(), key=lambda x: x[1], reverse=True):
    ...
    def main():
        ...
        Color.pl('  {C}• Network Type Classification:{W} Open/WEP/WPA/WPA2/WPA3/Enterprise/Guest/IoT')
        Color.pl('  {C}• Vendor Identification:{W} Apple/Samsung/Intel/Cisco/Huawei/Google/Microsoft')
        ...
        Color.pl('  {C}• Risk Assessment:{W} Environment security scoring (High/Medium/Low)')
        Color.pl('  {C}• Channel Analysis:{W} 2.4GHz/5GHz usage mapping and congestion')
        Color.pl('  {C}• MPI Parallelization:{W} Multi-core scanning for maximum coverage')
        Color.pl('  {C}• Intelligence Reporting:{W} Comprehensive statistics and insights')
