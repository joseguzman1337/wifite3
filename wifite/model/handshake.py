...
class Handshake(object):
    ...
    def divine_bssid_and_essid(self):
        ...
        if self.bssid is None:
            hs_regex = re.compile(
                r"^.*handshake_\w+_([0-9A-F\-]{17})_.*\.cap$", re.IGNORECASE
            )
        ...
        if not self.essid and not self.bssid:
            ...
            Color.pl(
                "{!} {O}Warning{W}: {O}Arbitrarily selected "
    ...
    def aircrack_handshakes(self):
        ...
        command = 'echo "" | aircrack-ng -a 2 -w - -b %s "%s"' % (
            self.bssid,
            self.capfile,
        )
    ...
    def print_pairs(pairs, capfile, tool=None):
        ...
        if len(pairs) == 0:
            Color.pl(
                "{!} %s.cap file {R}does not{O} contain a valid handshake{W}"
                % (tool_str)
            )
            return
