...
class Configuration:
    ...
    def initialize(cls, load_interface=True):
        ...
        cls.wps_pixie_timeout = (
            300  # Seconds to wait for PIN before WPS Pixie attack fails
        )
        ...
        cls.hashcat_realtime_wordlist_dir = (
            "/usr/share/wordlists/"  # Directory for wordlists
        )
    ...
    def parse_settings_args(cls, args):
        ...
        if args.random_mac:
            cls.random_mac = True
            Color.pl(
                "{+} {C}option:{W} using {G}random mac address{W} "
        ...
        if args.channel:
            ...
            Color.pl(
                "{+} {C}option:{W} scanning for targets on channel "
                + "{G}%s{W}" % args.channel
            )
        ...
        if args.interface:
            ...
            Color.pl(
                "{+} {C}option:{W} using wireless interface "
        ...
        if args.no_deauth == True:
            cls.no_deauth = True
            Color.pl(
                "{+} {C}option:{W} will {R}not{W} {O}deauth{W} clients "
        ...
        if args.num_deauths and args.num_deauths > 0:
            ...
            Color.pl(
                "{+} {C}option:{W} send {G}%d{W} deauth packets when deauthing"
                % (cls.num_deauths)
            )
        ...
        if args.clients_only == True:
            ...
            Color.pl(
                "{+} {C}option:{W} {O}ignoring targets that do not have "
                + "associated clients"
            )
        ...
        if args.scan_time:
            ...
            Color.pl(
                "{+} {C}option:{W} ({G}pillage{W}) attack all targets "
    ...
    def parse_wep_args(cls, args):
        ...
        if args.wep_pps:
            ...
            Color.pl(
                "{+} {C}option:{W} using {G}%d{W} packets/sec on WEP attacks"
                % (args.wep_pps)
            )
        ...
        if args.wep_timeout:
            ...
            Color.pl(
                "{+} {C}option:{W} WEP attack timeout set to "
        ...
        if args.require_fakeauth:
            ...
            Color.pl(
                "{+} {C}option:{W} fake-authentication is "
        ...
        if args.wep_crack_at_ivs:
            ...
            Color.pl(
                "{+} {C}option:{W} will start cracking WEP keys at "
        ...
        if args.wep_restart_aircrack:
            ...
            Color.pl(
                "{+} {C}option:{W} will restart aircrack every "
    ...
    def parse_wpa_args(cls, args):
        ...
        if args.wordlist:
            if not os.path.exists(args.wordlist):
                ...
                cls.wordlist = None
                Color.pl(
                    "{+} {C}option:{O} wordlist {R}%s{O} is a directory, not a file. Wifite will NOT attempt to crack handshakes"
                    % args.wordlist
                )
        ...
        if args.wpa_deauth_timeout:
            ...
            Color.pl(
                "{+} {C}option:{W} will deauth WPA clients every "
        ...
        if args.wpa_attack_timeout:
            ...
            Color.pl(
                "{+} {C}option:{W} will stop WPA handshake capture after "
        ...
        if args.ignore_old_handshakes:
            ...
            Color.pl(
                "{+} {C}option:{W} will {O}ignore{W} existing handshakes "
                + "(force capture)"
            )
        ...
        if args.wpa_handshake_dir:
            ...
            Color.pl(
                "{+} {C}option:{W} will store handshakes to "
    ...
    def parse_wps_args(cls, args):
        ...
        if args.wps_only:
            ...
            Color.pl(
                "{+} {C}option:{W} will *only* attack WPS networks with "
        ...
        if args.no_wps:
            ...
            cls.wps_pin = False
            Color.pl(
                "{+} {C}option:{W} will {O}never{W} use {C}WPS attacks{W} "
            ...
            cls.wps_pin = False
            Color.pl(
                "{+} {C}option:{W} will {G}only{W} use {C}WPS Pixie-Dust "
            ...
            cls.wps_pin = True
            Color.pl(
                "{+} {C}option:{W} will {G}only{W} use {C}WPS PIN attack{W} "
        ...
        if args.use_bully:
            ...
            if not Bully.exists():
                ...
                Color.pl(
                    "{+} {C}option:{W} use {C}bully{W} instead of {C}reaver{W} "
                    + "for WPS Attacks"
                )
        ...
        if args.wps_pixie_timeout:
            ...
            Color.pl(
                "{+} {C}option:{W} WPS pixie-dust attack will fail after "
        ...
        if args.wps_fail_threshold:
            ...
            Color.pl(
                "{+} {C}option:{W} will stop WPS attack after "
    ...
    def parse_pmkid_args(cls, args):
        if args.use_pmkid_only:
            ...
            Color.pl(
                "{+} {C}option:{W} will ONLY use {C}PMKID{W} attack on WPA networks"
            )

        if args.pmkid_timeout:
            cls.pmkid_timeout = args.pmkid_timeout
            Color.pl(
                "{+} {C}option:{W} will wait {G}%d seconds{W} during {C}PMKID{W} capture"
                % args.pmkid_timeout
            )
    ...
    def parse_hashcat_realtime_args(cls, args):
        ...
        if (
            ...
            Color.pl(
                ...
                % args.hashcat_realtime_options
            )

        if (
            hasattr(args, "hashcat_realtime_force_cpu")
            and args.hashcat_realtime_force_cpu
        ):
    ...
    def parse_wep_attacks(cls):
        ...
        if len(cls.wep_attacks) == 0:
            ...
            Color.pl(
                "{+} {C}option:{W} using {G}%s{W} WEP attacks"
    ...
    def exit_gracefully(cls, code=0):
        ...
        if Airmon.killed_network_manager:
            Color.pl(
                "{!} You can restart NetworkManager when finished ({C}service network-manager start{W})"
            )
    ...
    def dump(cls):
        ...
        for key, val in sorted(cls.__dict__.items()):
            if (
                key.startswith("__")
                or type(val) in [classmethod, staticmethod]
                or val is None
            ):
                continue
            result += Color.s(
