...
class CrackHelper:
    ...
    TYPES = {"4-WAY": "4-Way Handshake", "PMKID": "PMKID Hash"}
    ...
    def run(cls):
        ...
        if not Configuration.wordlist:
            ...
            if not os.path.exists(Configuration.wordlist):
                Color.pl(
                    "{!} {R}Wordlist {O}%s{R} not found. Exiting."
        ...
        if all_pmkid:
            Color.pl(
                "{!} {O}Note: PMKID hashes can only be cracked using {C}hashcat{W}"
            )
            tool_name = "hashcat"
        else:
            Color.p(
            ...
            if tool_name not in available_tools:
                Color.pl(
                    '{!} {R}"%s"{O} tool not found, defaulting to {C}aircrack{W}'
                    % tool_name
                )
                tool_name = "aircrack"
            ...
            for hs in hs_to_crack:
                if tool_name != "hashcat" and hs["type"] == "PMKID":
                    if "hashcat" in missing_tools:
                        Color.pl(
                            "{!} {O}Hashcat is missing, therefore we cannot crack PMKID hash{W}"
                        )
    ...
    def crack(cls, hs, tool):
        ...
        if crack_result is None:
            # Failed to crack
            Color.pl(
                "{!} {R}Failed to crack {O}%s{R} ({O}%s{R}): Passphrase not in dictionary"
                % (hs["essid"], hs["bssid"])
            )
        else:
            # Cracked, replace existing entry (if any), or add to
            Color.pl(
                '{+} {G}Cracked{W} {C}%s{W} ({C}%s{W}). Key: "{G}%s{W}"'
