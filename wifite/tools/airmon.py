...
class AirmonIface(object):
    ...
    def menu_header():
        ...
        s += "\n"
        s += "-" * (
            AirmonIface.INTERFACE_LEN
            + AirmonIface.PHY_LEN
            + AirmonIface.DRIVER_LEN
            + AirmonIface.CHIPSET_LEN
            + 3
        )
        return s
...
class Airmon(Dependency):
    ...
    def terminate_conflicting_processes():
        ...
        for pid, pname in pid_pnames:
            if pname == "NetworkManager" and Process.exists("service"):
                Color.pl(
                    "{!} {O}stopping network-manager ({R}service network-manager stop{O})"
                )
                ...
                Color.pl(
                    "{!} {O}stopping avahi-daemon ({R}service avahi-daemon stop{O})"
                )
                # Can't just pkill avahi-daemon; it's a service
                Process(["service", "avahi-daemon", "stop"]).wait()
            else:
                Color.pl(
                    "{!} {R}Terminating {O}conflicting process {R}%s{O} (PID {R}%s{O})"
                    % (pname, pid)
                )
                try:
    ...
    def start_network_manager():
        ...
        if Process.exists("systemctl"):
            ...
            Color.pl(
                " {R}cannot restart NetworkManager: {O}systemctl{R} or {O}service{R} not found{W}"
            )
