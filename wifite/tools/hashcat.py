...
class HcxPcapTool(Dependency):
    ...
    def generate_hccapx_file(handshake, show_command=False):
        ...
        command = ["hcxpcaptool", "-o", hccapx_file, handshake.capfile]
    ...
    def generate_john_file(handshake, show_command=False):
        ...
        command = ["hcxpcaptool", "-j", john_file, handshake.capfile]
    ...
    def get_pmkid_hash(self, pcapng_file):
        ...
        command = ["hcxpcaptool", "-z", self.pmkid_file, pcapng_file]
    ...
    def start_realtime_crack(
        ...
        """
        if not Configuration.hashcat_path or not os.path.exists(
            Configuration.hashcat_path
        ):
            Color.pl(
                "{!} {R}Hashcat path not configured or not found ({O}%s{R}). Cannot start cracking.{W}"
                % Configuration.hashcat_path
            )
            return None
        ...
        if not os.path.exists(hash_file_path):
            Color.pl(
                "{!} {R}Hash file not found: {O}%s{R}. Cannot start cracking.{W}"
                % hash_file_path
            )
            return None
        ...
        if not os.path.exists(wordlist_path):
            Color.pl(
                "{!} {R}Wordlist not found: {O}%s{R}. Cannot start cracking.{W}"
                % wordlist_path
            )
            return None
        ...
        hashcat_cmd = [
            ...
            "--status",
            "--status-timer",
            "5",  # Update status every 5 seconds
            "--session",
            session_name,
        ]
        ...
        if user_preferences:
            ...
            if "opencl_device_types" in user_preferences:
                hashcat_cmd.extend(
                    [
                        "--opencl-device-types",
                        user_preferences["opencl_device_types"],
                    ]
                )
        ...
        try:
            ...
            Color.pl(
                "{+} {C}Starting Hashcat session {O}%s{C} for {O}%s{W}"
            ...
            Color.pl(
                "{!} {R}Hashcat executable not found at {O}%s{W}"
                % Configuration.hashcat_path
            )
            return None
    ...
    def check_realtime_crack_status(session: RealtimeHashcatSession):
        ...
        while (
            session.popen_object.stdout
            and select.select([session.popen_object.stdout], [], [], 0)[0]
        ):
        ...
        while (
            session.popen_object.stderr
            and select.select([session.popen_object.stderr], [], [], 0)[0]
        ):
        ...
        if (
            os.path.exists(session.outfile_path)
            and os.path.getsize(session.outfile_path) > 0
        ):
            try:
                with open(session.outfile_path, "r") as f:
                    ...
                    if line:
                        ...
                        if len(parts) >= 2:  # At least hash:plain
                            ...
                            if (
                                session.hash_type == 2500 and len(parts) > 1
                            ):  # HCCAPX often has ESSID as first part
                                cracked_password = parts[
                                    -1
                                ]  # Assume password is last part
                            elif (
                                session.hash_type != 2500 and len(parts) > 0
                            ):  # For PMKID etc.
    ...
    def stop_realtime_crack(session: RealtimeHashcatSession, cleanup_hash_file=False):
        ...
        if session and session.popen_object:
            if session.popen_object.poll() is None:  # Process is still running
                try:
                    ...
                    session.popen_object.wait(
                        timeout=3
                    )  # Wait for graceful termination
                except ProcessLookupError:
                    Color.pl(
                        "{!} {O}Hashcat process group for {C}%s{O} already gone.{W}"
                        % session.target_bssid
                    )
                except subprocess.TimeoutExpired:
                    Color.pl(
                        "{!} {O}Hashcat session for {C}%s{O} did not terminate gracefully, sending SIGKILL...{W}"
                        % session.target_bssid
                    )
                    try:
                        os.killpg(
                            os.getpgid(session.popen_object.pid),
                            signal.SIGKILL,
                        )
        ...
        if session.outfile_path and os.path.exists(session.outfile_path):
            try:
                ...
                Color.pl(
                    "{!} {R}Could not remove outfile {O}%s{R}: {O}%s{W}"
                    % (session.outfile_path, str(e))
                )
        ...
        if session.potfile_path and os.path.exists(session.potfile_path):
            try:
                ...
                Color.pl(
                    "{!} {R}Could not remove potfile {O}%s{R}: {O}%s{W}"
                    % (session.potfile_path, str(e))
                )

        if (
            cleanup_hash_file
            and session.hash_file_path
            and os.path.exists(session.hash_file_path)
        ):
            ...
            if Configuration.temp() in os.path.abspath(session.hash_file_path):
                try:
                    ...
                    Color.pl(
                        "{!} {R}Could not remove temp hash file {O}%s{R}: {O}%s{W}"
                        % (session.hash_file_path, str(e))
                    )
            else:
                Color.pl(
                    "{!} {O}Skipping cleanup of non-temporary hash file: {C}%s{W}"
                    % session.hash_file_path
                )

        Color.pl(
            "{+} {C}Hashcat session for {O}%s{C} stopped and cleaned up.{W}"
            % session.target_bssid
        )
