...
class Airodump(Dependency):
    ...
    def find_files(self, endswith=None):
        return self.find_files_by_output_prefix(
            self.output_file_prefix, endswith=endswith
        )
    ...
    def delete_airodump_temp_files(cls, output_file_prefix):
        ...
        for fil in os.listdir("."):
            if (
                fil.startswith("replay_")
                and fil.endswith(".cap")
                or fil.endswith(".xor")
            ):
                os.remove(fil)
    ...
    def deauth_hidden_targets(self):
        ...
        for target in self.targets:
            ...
            if Configuration.verbose > 1:
                ...
                Color.pe(
                    "{C} [?] Deauthing %s (broadcast & %d clients){W}"
