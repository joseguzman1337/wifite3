...
class RealtimeCrackManager:
    def __init__(self, config):
        ...
        self.current_hash_file_path = (
            None  # Path to the file Hashcat is currently working on
        )
    ...
    def _try_next_wordlist(self):
        ...
        custom_options = (
            ...
            if Configuration.hashcat_realtime_options
            else []
        )
    ...
    def update_status(self):
        ...
        for line in status_info["status_lines"]:
            ...
            if (
                ...
                or "SPEED" in line.upper()
                or "PROGRESS" in line.upper()
                or "RECOVERED" in line.upper()
                or "REJECTED" in line.upper()
        ...
        if status_info["cracked_password"]:
            ...
            Color.pl(
                f"{{G}}SUCCESS: Real-time crack for {{C}}{self.current_target_bssid}{{G}} (ESSID: {self.current_target_essid})! Password: {{R}}{password}{{W}}"
            )
    ...
    def stop_current_crack_attempt(self, cleanup_hash_file=False):
        if self.active_session:
            Color.pl(
                f"{{G}}Real-time: Stopping session for {{C}}{self.current_target_bssid or 'N/A'}{{W}}."
            )
            ...
            Hashcat.stop_realtime_crack(
                self.active_session,
                cleanup_hash_file=should_cleanup_session_hash_file,
            )
        ...
        if (
            cleanup_hash_file
        ):  # This implies we are done with this BSSID or found password
            self.current_target_bssid = None
            self.current_hash_file_path = None
            self.current_hash_type = None
