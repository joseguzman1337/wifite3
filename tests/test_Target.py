...
class TestTarget(unittest.TestCase):
    ...
    def test_wpa3_parsing(self):
        ...
        self.assertFalse(
            target.is_owe
        )  # OWE is in privacy but WPA3 takes precedence for is_wpa3, encryption should be WPA3
    ...
    def test_wifi_standard_parsing(self):
        ...
        self.assertEqual(
            target.wifi_standard, "ac"
        )  # Current logic pushes >=300 to 'ac'
    ...
    def test_target_to_str_formatting(self):
        ...
        target_str = Color.strip(
            target_wpa3_ax.to_str()
        )  # Strip color codes for simple substring check
        ...
        self.assertIn(
            Color.s("{R}WPA3"), target_str_wpa3_color
        )  # Check if WPA3 is wrapped in Red color

        target_str_owe_color = target_owe_ac.to_str()
        self.assertIn(
            Color.s("{M} OWE"), target_str_owe_color
        )  # Check if OWE is wrapped in Magenta (note space due to rjust(4))

        target_str_ax_color = target_wpa3_ax.to_str()
        self.assertIn(
            Color.s("{C}[AX]"), target_str_ax_color
        )  # Check if [AX] is wrapped in Cyan
