...
class TestAttackWPA(unittest.TestCase):
    ...
    def test_run_on_wpa3_target(self, mock_stdout):
        ...
        result = attack.run()
        self.assertFalse(
            result, "AttackWPA.run() should return False for WPA3 targets."
        )
        ...
        self.assertIn(
            expected_msg_part,
            Color.strip(output),
            "WPA3 warning message not found in stdout.",
        )
    ...
    def test_run_on_wpa2_target_proceeds(self, mock_color_pl, mock_capture_handshake):
        ...
        self.assertFalse(
            result
        )  # It will be False because capture_handshake returns None
    ...
    @patch(
        "wifite.tools.aircrack.Aircrack.crack_handshake"
    )  # Mock aircrack-ng cracking
    @patch("os.path.exists", return_value=True)  # Assume files exist
    def test_run_starts_realtime_crack_on_wpa2_target_if_enabled(
        self,
        mock_path_exists,
        mock_aircrack_crack,
        mock_gen_hccapx,
        ...
        mock_handshake_obj.capfile = (
            "/tmp/fake_handshake.cap"  # Used by generate_hccapx_file
        )
        mock_handshake_obj.bssid = target_wpa2.bssid
        mock_handshake_obj.essid = target_wpa2.essid
        mock_handshake_obj.has_handshake.return_value = (
            True  # Simulate successful capture
        )

        with patch.object(
            attack, "capture_handshake", return_value=mock_handshake_obj
        ) as mock_capture:
